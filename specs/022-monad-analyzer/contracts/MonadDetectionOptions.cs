...
public sealed record MonadDetectionOptions
{
    ...
    /// </summary>
    public HashSet<MonadType> EnabledTypes { get; init; } = new()
    {
        MonadType.Option,
        MonadType.Either,
    ...
    public static MonadDetectionOptions Parse(
        Microsoft.CodeAnalysis.Diagnostics.AnalyzerConfigOptions analyzerConfigOptions)
    {
        // Parse enabled flag
        var enabled = false;
        ...
        {
            if (int.TryParse(complexityValue, out var parsed) && parsed > 0)
            {
        ...
        {
            enabledTypes.Clear();
            var monadNames = monadsValue.Split(',', StringSplitOptions.RemoveEmptyEntries);
