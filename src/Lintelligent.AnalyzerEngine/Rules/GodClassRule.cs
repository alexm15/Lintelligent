...
public class GodClassRule : IAnalyzerRule
{
    ...
    public IEnumerable<DiagnosticResult> Analyze(SyntaxTree tree)
    {
        ...
        foreach (var classDecl in classes)
        {
            ...
            if (lineCount > MaxLines || methodCount > MaxMethods)
            {
                ...
                if (lineCount > MaxLines && methodCount > MaxMethods)
                {
                    message = $"Class '{className}' has {lineCount} lines (max: {MaxLines}) and {methodCount} methods (max: {MaxMethods}). " +
                             "Consider splitting into smaller, focused classes by responsibility.";
                }
                else if (lineCount > MaxLines)
                {
                    message = $"Class '{className}' has {lineCount} lines (max: {MaxLines}). " +
                             "Consider splitting into smaller, focused classes by responsibility.";
                }
                else // methodCount > MaxMethods
                {
                    message = $"Class '{className}' has {methodCount} methods (max: {MaxMethods}). " +
                             "Consider splitting into smaller, focused classes by responsibility.";
                }
