...
public static class StatementSequenceExtractor
{
    ...
    private static IEnumerable<StatementSequence> ExtractFromMethods(SyntaxNode root, string filePath, int minStatements)
    {
        foreach (var method in root.DescendantNodes().OfType<MethodDeclarationSyntax>())
        {
            if (method.Body != null)
            {
                ...
                foreach (var seq in ExtractSequencesFromBlock(method.Body, context, filePath, minStatements))
                    yield return seq;
            }
        }
    }
    
    private static IEnumerable<StatementSequence> ExtractFromConstructors(SyntaxNode root, string filePath, int minStatements)
    {
        foreach (var constructor in root.DescendantNodes().OfType<ConstructorDeclarationSyntax>())
        {
            if (constructor.Body != null)
            {
                ...
                foreach (var seq in ExtractSequencesFromBlock(constructor.Body, context, filePath, minStatements))
                    yield return seq;
            }
        }
    }
    
    private static IEnumerable<StatementSequence> ExtractFromAccessors(SyntaxNode root, string filePath, int minStatements)
    {
        foreach (var accessor in root.DescendantNodes().OfType<AccessorDeclarationSyntax>())
        {
            if (accessor.Body != null)
            {
                ...
                foreach (var seq in ExtractSequencesFromBlock(accessor.Body, context, filePath, minStatements))
                    yield return seq;
            }
        }
    }
    
    private static IEnumerable<StatementSequence> ExtractFromCatchBlocks(SyntaxNode root, string filePath, int minStatements)
    {
        foreach (var catchClause in root.DescendantNodes().OfType<CatchClauseSyntax>())
        {
            if (catchClause.Block != null)
            {
                ...
                foreach (var seq in ExtractSequencesFromBlock(catchClause.Block, context, filePath, minStatements))
                    yield return seq;
            }
        }
    }
    
    private static IEnumerable<StatementSequence> ExtractFromTryBlocks(SyntaxNode root, string filePath, int minStatements)
    {
        foreach (var tryStatement in root.DescendantNodes().OfType<TryStatementSyntax>())
        {
            if (tryStatement.Block != null)
            {
                ...
                foreach (var seq in ExtractSequencesFromBlock(tryStatement.Block, context, filePath, minStatements))
                    yield return seq;
            }
        }
    }
        ...
        int minStatements)
    {
        ...
        {
            foreach (var seq in ExtractFromBlock(block, context, filePath, minStatements))
                yield return seq;
        }
    }
        ...
        int minStatements)
    {
        ...
        for (var startIndex = 0; startIndex <= statements.Count - minStatements; startIndex++)
        {
            ...
            {
                var sequence = statements.Skip(startIndex).Take(length).ToArray();
