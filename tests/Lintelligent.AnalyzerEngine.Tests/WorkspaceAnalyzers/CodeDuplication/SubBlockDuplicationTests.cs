...
public class SubBlockDuplicationTests
{
    ...
    public void FindDuplicates_ValidationLogicInTwoMethods_DetectsDuplication()
    {
        ...
        SyntaxTree tree = CSharpSyntaxTree.ParseText(code, path: "UserService.cs");
        var options = new DuplicationOptions(3, 10);

        // Act
        var groups = ExactDuplicationFinder.FindDuplicates(new[] {tree}, options).ToList();

        // Assert
        DuplicationGroup? validationDup = groups.FirstOrDefault(g =>
            g.Instances.Count == 2 &&
    ...
    public void FindDuplicates_ErrorHandlingInDifferentMethods_DetectsDuplication()
    {
        ...
        SyntaxTree tree = CSharpSyntaxTree.ParseText(code, path: "ApiClient.cs");
        var options = new DuplicationOptions(4, 15);

        // Act
        var groups = ExactDuplicationFinder.FindDuplicates(new[] {tree}, options).ToList();

        // Assert - The catch block body (statements inside catch) should be detected
        DuplicationGroup? errorHandlingDup = groups.FirstOrDefault(g =>
            g.Instances.Count == 2 &&
            g.Instances.Any(i => i.SourceText.Contains("_logger.LogError")));

        errorHandlingDup.Should()
            .NotBeNull("because the error handling statements are duplicated in both catch blocks");
    ...
    public void FindDuplicates_TransactionPatternAcrossMethods_DetectsDuplication()
    {
        ...
        SyntaxTree tree = CSharpSyntaxTree.ParseText(code, path: "OrderRepository.cs");
        var options = new DuplicationOptions(3, 10);
        ...
        DuplicationGroup? transactionDup = groups.FirstOrDefault(g =>
            g.Instances.Count == 3 &&
    ...
    public void FindDuplicates_NoSubBlockDuplication_ReturnsEmpty()
    {
        ...
        SyntaxTree tree = CSharpSyntaxTree.ParseText(code, path: "Calculator.cs");
        var options = new DuplicationOptions(3, 10);
    ...
    public void FindDuplicates_ShortSequence_NotDetected()
    {
        ...
        SyntaxTree tree = CSharpSyntaxTree.ParseText(code, path: "Service.cs");
        var options = new DuplicationOptions(3, 10);
